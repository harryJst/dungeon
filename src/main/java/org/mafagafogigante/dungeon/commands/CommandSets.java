...
final class CommandSets {
  ...
  @NotNull
  private Map<String, CommandSet> initializeCommandSetMap(Game g) {
    Map<String, CommandSet> map = new HashMap<>();
    map.put("default", initializeDefaultCommandSet(g));
    map.put("extra", initializeExtraCommandSet(g.getGameState()));
    map.put("debug", initializeDebugCommandSet(g.getGameState()));
    return map;
  }

  private CommandSet initializeDefaultCommandSet(final Game g) {
    ...
    commandSet.addCommand(new Command("look", "Describes what the character can see.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        g.getGameState().getHero().look(g.getGameState());
      }
    });
    commandSet.addCommand(new Command("map", "Shows a map of your surroundings.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        WorldMapWriter.writeMap(g.getGameState());
      }
    });
    commandSet.addCommand(new Command("milk", "Attempts to milk a creature.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        g.getGameState().getHero().parseMilk(arguments,g.getGameState().getEngine());
      }
    });
    ...
    commandSet.addCommand(new Command("unequip", "Unequips the currently equipped item.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        g.getGameState().getHero().unequipWeapon(g.getGameState().getEngine());
      }
    });
  ...
  private static CommandSet initializeExtraCommandSet(final GameState gameState) {
    CommandSet commandSet = CommandSet.emptyCommandSet();
    ...
    commandSet.addCommand(new Command("statistics", "Displays all available game statistics.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        gameState.getStatistics().writeStatistics();
      }
    });
  ...
  private CommandSet initializeDebugCommandSet(final GameState gameState) {
    CommandSet commandSet = CommandSet.emptyCommandSet();
    commandSet.addCommand(new Command("achievements", "Writes the achievements you have not yet unlocked.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        AchievementTracker tracker = gameState.getHero().getAchievementTracker();
    ...
    commandSet.addCommand(new Command("kills", "Writes statistics about your killings.") {
      ...
      public void execute(@NotNull String[] arguments) {
        CounterMap<CauseOfDeath> map =
            gameState.getStatistics().getBattleStatistics().getKillsByCauseOfDeath();
        if (map.isNotEmpty()) {
    ...
    commandSet.addCommand(new Command("location", "Writes information about the current location.") {
      ...
      public void execute(@NotNull String[] arguments) {
        ...
        Location heroLocation = gameState.getHero().getLocation();
    ...
    commandSet.addCommand(new Command("map", "Produces a map as complete as possible.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        WorldMapWriter.writeDebugMap(gameState);
      }
    });
        ...
        if (arguments.length != 0) {
          ...
          try {
            ...
            if (world.getItemFactory().canMakeItem(id)) {
              ...
              gameState.getEngine().refresh(); // Set the game state to unsaved after adding an item to the world.
            } else {
      ...
      public void execute(@NotNull String[] arguments) {
        if (arguments.length != 0) {
          for (String argument : arguments) {
            ...
            World world = gameState.getWorld();
            Creature creature = world.getCreatureFactory().makeCreature(givenId, world,gameState);
            if (creature != null) {
              gameState.getHero().getLocation().addCreature(creature);
              Writer.write("Spawned a " + creature.getName() + ".");
              gameState.getEngine().refresh(); // Set the game state to unsaved after adding a creature to the world.
            } else {
    ...
    commandSet.addCommand(new Command("time", "Writes information about the current time.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        Writer.write(gameState.getWorld().getWorldDate().toString());
      }
    });
    commandSet.addCommand(new Command("wait", "Makes time pass.") {
      @Override
      public void execute(@NotNull String[] arguments) {
        debugWaitParser.parseDebugWait(arguments,gameState);
      }
    });
    return commandSet;
  }
